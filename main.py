...
def preprocess(text):
    ...
    tokens = [t for t in tokens
              ...
              and t not in DOMAIN_NOISE]   # <-- extra filter
    return [lemmatiser.lemmatize(t) for t in tokens]


def _score_category(tokens: list[str], token_set: set[str], raw_text: str) -> float:
    """
    Simple scoring:
      +1 for each token that appears in the keyword set
      +0.5 bonus for each keyword phrase found in the raw text
    Returns the raw hit count (caller decides thresholds).
    """
    score = sum(1 for t in tokens if t in token_set)
    # Also check for multi-word keywords in raw text
...
def categorise_concepts(tokens: list[str], raw: str) -> list[str]:
    """Return a ranked list of core concepts detected."""
    scores = {
        concept: _score_category(tokens, kws, raw)
        for concept, kws in CONCEPT_KEYWORDS.items()
    }
    detected = sorted(
        [(concept, s) for concept, s in scores.items() if s > 0],
        key=lambda x: x[1],
        reverse=True,
    )
    return [concept for concept, _ in detected]
