...
IMPLEMENTATION_KEYWORDS = {
    k: stem_set(v) for k, v in PLAIN_IMPLEMENTATION_KEYWORDS.items()
}
...
def preprocess(text):
    ...
    tokens = [t for t in tokens if t not in stop_words and t not in DOMAIN_NOISE]
...
def categorise_implementation(tokens, raw):
    scores = {
        impl: score_category(tokens, kws, raw)
